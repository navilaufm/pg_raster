 ...
 #postgis 
    ...
    ## delete older of same value
     psql -h "$dbhost" -U "$dbuser" -d "$dbname" -p "$dbport" -c "
      delete from chirps_data where load_datetime<=now() and variable =   '$variable';
    "
...
for file in $files; do
  ...
  # Para obtener YYMMDD
  year_month=$(echo $filename | sed 's/.*\([0-9]\{4\}\.[0-9]\{2\}\).*/\1/')
...
##set -x
raster2pgsql -s 4326 -a "$download_dir/$filename" chirps_data | psql -h "$dbhost" -U "$dbuser" -d "$dbname" -p "$dbport"
